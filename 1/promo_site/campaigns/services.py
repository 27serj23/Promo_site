...
class ServiceError(Exception):
    """Ошибка бизнес-логики. Отличается от ValueError, чтобы не перехватывать баги."""
...
def create_campaign(name, owner):
    ...
    if not name or not name.strip():
        raise ServiceError('Название кампании не может быть пустым')
    if not owner:
        raise ServiceError('Владелец кампании обязателен')
...
def get_campaign_for_user(campaign_id, user):
    ...
    Если пользователь не владелец и не участник — ошибка доступа.
    """
    if not campaign_id:
        raise ServiceError('ID кампании обязателен')
...
def get_campaign_detail_data(campaign):
    ...
    if not campaign:
        raise ServiceError('Кампания обязательна')
...
def add_house_to_campaign(campaign, city, street, house_number, entrances, apartments_per_entrance):
    ...
    if not city or not city.strip():
        raise ServiceError('Город обязателен')
    if not street or not street.strip():
        raise ServiceError('Улица обязательна')
    if not house_number or not house_number.strip():
        raise ServiceError('Номер дома обязателен')
    if entrances < 1:
        raise ServiceError('Количество подъездов должно быть положительным')
    if apartments_per_entrance < 1:
        raise ServiceError('Количество квартир в подъезде должно быть положительным')
...
def get_house_with_visits(campaign, house_id, page_number=1, per_page=10):
    ...
    if not campaign:
        raise ServiceError('Кампания обязательна')
    if not house_id:
        raise ServiceError('ID дома обязателен')
...
def record_visit(house, visitor, entrance, apartment_number, opened_door,
    ...
    if not house:
        raise ServiceError('Дом обязателен')
    if not visitor:
        raise ServiceError('Посетитель обязателен')
    if entrance < 1:
        raise ServiceError('Номер подъезда должен быть положительным')
    if apartment_number < 1:
        raise ServiceError('Номер квартиры должен быть положительным')
    ...
    return ApartmentVisit.objects.create(
        ...
        contact_name=contact_name.strip() if isinstance(contact_name, str) else None,
        contact_phone=contact_phone.strip() if isinstance(contact_phone, str) else None,
